...
with open(r'configManager.json', 'r') as f:
    json_data = json.load(f)
    global prefix, extensions, login, psswd, db, host_ip
...
async def on_ready():
    print("Dołączyłem.")
    await client.change_presence(activity=discord.Activity(type=discord.ActivityType.listening, name=f'n!help | {len(client.guilds)} serwerów.'))

@client.command()
async def konfiguruj(ctx):
    if ctx.message.author.id == ctx.guild.owner_id:
        await ctx.guild.create_text_channel('🔔narsty')
    else:
        await ctx.send("Nie masz dostępu do tej komendy! Tylko własciciel serwera ma uprawnienia.")

@client.event
async def on_guild_join(guild):
    channel = discord.utils.find(lambda r: r.name == '🔔narsty', guild.text_channels)
    if channel in guild:
        pass
    else:
        await guild.create_text_channel('🔔narsty')
...
async def load(ctx, extension):
    if ctx.message.author.id == 318824628439089152:
        ...
        embed = discord.Embed(title="Narsty", description="Nie masz dostępu do tej komendy!", color=discord.Color.dark_red())
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.CommandNotFound):
        ...
        embed = discord.Embed(title="Narsty", description=f"{ctx.author.mention}, masz już utworzoną ofertę na (może innym) serwerze.", color=discord.Color.orange())
        await ctx.send(embed=embed)
    elif isinstance(error, asyncpg.UniqueViolationError):
        embed = discord.Embed(title="Narsty", description=f"{ctx.author.mention}, masz już utworzoną ofertę na (może innym) serwerze.", fcolor=discord.Color.orange())
        await ctx.send(embed=embed)
    else:
        raise error
